...
export default function BoardThemePicker({ value, onChange, layout = "strip" }: BoardThemePickerProps) {
  if (layout === "grid") {
    return (
      <div>
        <span className="text-[10px] uppercase tracking-wider text-[#888] font-semibold">Board theme</span>
              ...
              <button
                ...
                className={`flex flex-col items-center gap-0.5 p-1 rounded-md transition-all ${
                  active
                    ...
                    : "ring-1 ring-[#5a5652] hover:ring-[#81b64c]/50 bg-[#363430]"
                }`}
              >
                <div
                  className="w-full aspect-square rounded overflow-hidden border border-black/30"
                  style={{
                ...
                />
                <span className="text-[8px] text-[#aaa] truncate w-full text-center">{theme.emoji}</span>
              </button>
            )
          })}
        </div>
